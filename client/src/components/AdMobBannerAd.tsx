    ...
    const initializeAd = async () => {
      try {
        ...
        if (isCapacitor && typeof window !== 'undefined' && (window as any).Capacitor) {
          // Only import native library when actually in Capacitor environment
          try {
            // This will only work in actual native builds, not web builds
            console.log('Native AdMob will initialize with unit ID:', adUnitId);
          } catch (error) {
            console.log('Native AdMob not available in this environment');
          }
        } else {
